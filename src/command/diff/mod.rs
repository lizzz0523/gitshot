...
fn annotate_inline_diffs(lines: &mut [(char, String, Vec<Range<usize>>)]) {
    let n = lines.len();
    let mut i = 0;
    ...
    while i < n {
        ...
        for k in 0..pair_count {
            let di = del_start + k;
            let ai = add_start + k;
            let (del_ranges, add_ranges) = emphasizer::emphasis(&lines[di].1, &lines[ai].1);
...
pub fn run(config: &Config, paths: &[String], whitespace: bool) {
    ...
    let mut raw_lines: Vec<(char, String, Vec<Range<usize>>)> = Vec::new();
...
fn line_color(line: &DiffLine, text: &str, diff: &DiffStyle) -> Color {
    match line.origin {
        '+' => diff.added_fg,
        '-' => diff.deleted_fg,
        'H' => diff.hunk_fg,
